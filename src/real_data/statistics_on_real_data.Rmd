...
Making maps from an occurrence cube is quite wrong as each occurrence cube is different from each other due to the random assignment. So, this map is misleading:
...
as actually the circles are way larger than the grid resolution:
...
cells_examples <- get_cells_around_circles(
  grid = be_grid_4326, 
...
leaflet() %>%
  addTiles() %>%
  setView(lng = 3.12536, lat = 50.941963, zoom = 10) %>%
  addPolygons(data = circles_example,
              fillColor = "blue",
              weight = 1, 
              opacity = 0.2,
              fillOpacity = 0.1) %>%
  addPolygons(data = cells_examples,
              fillColor = "green",
              weight = 3,
              color = "green",
              opacity = 0.8)
