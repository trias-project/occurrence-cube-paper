...
source(here("src", "simulations", "simulation_functions.R"))
```
...
```{r, fig.height=10}
final_occurrence_df %>%
  rename(`number of occurrences` = counting) %>%
  pivot_longer(cols = c(expected_n, `number of occurrences`), 
               names_to = "measure", values_to = "n") %>%
  st_as_sf() %>%
  ggplot() +
    geom_sf(aes(fill = n)) +
...
We can for example model the average number of occurrences over the entire grid per year assuming a zero-inflated Poisson distribution.
...
The estimated abundance over the complete grid is not significantly different between the two years.
...
Do we really need to simulate 1000 cubes?
Can we work with less?
We look at the 2022 data.
...
```{r}
dir.create(here("src", "simulation_study_cache"), showWarnings = FALSE)
file <- here("src", "simulation_study_cache", "bootstrapped_mean.Rds")
if (file.exists(file)) {
  cum_mean_df2 <- readRDS(file)
} else {
  ...
  saveRDS(cum_mean_df2, file = file)
}
```
...
We zoom around the thresholds 0.3 - 0.5 which result in an observed occupancy of 7 cells, found as the mean value (see histogram above):

```{r}
cum_occupancy_df %>%
  mutate(occupancy_30 = ifelse(p >= 0.3, 1, 0),
         occupancy_40 = ifelse(p >= 0.4, 1, 0),
         occupancy_50 = ifelse(p >= 0.5, 1, 0)) %>% 
  group_by(sim) %>%
  summarise(across(starts_with("occupancy_"), sum)) %>%
  ...
  ggplot(aes(x = sim, y = n, colour = threshold, group = threshold)) +
  geom_line(size = 1.5) +
  scale_x_log10(breaks = c(0,1,2,5,10,30,50,100,1000)) +
