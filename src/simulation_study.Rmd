...
```{r}
test_points2 <- sim_points(10, seed = 1234)

test_circles2 <- test_points2 %>%
  st_buffer(test_points$coordinateUncertaintyInMeters)

grid_df2 <- create_grid(500, test_circles2)

final_df2 <- sim_cubes(sims, test_points2, grid_df2)
...
1.    Based on the distribution of occupied grid cells we see that 7 occupied grid cells is the most probable
...
2.   We select the 7 grid cells with the highest probability of occupancy

```{r}
full_join(grid_df, occupancy_prob_df, by = join_by(id)) %>%
  mutate(p = ifelse(p < 0.3, NA, p)) %>%
  ggplot() +
  geom_sf(aes(fill = p)) +
  geom_sf_text(aes(label = round(p, 3))) +
  geom_sf(data = test_circles, fill = alpha("white", 0), 
...
3.    These cells need to have 11 occurrences in total. We get this by rounding up the expected number of occurrences in the selected grid cells

```{r}
summary_grid_props %>%
  mutate(expected_n = ifelse(expected_n < 0.4, NA, expected_n),
         n = ifelse(is.na(expected_n), 0, ceiling(expected_n))) %>%
  pivot_longer(cols = c(expected_n, n), 
               names_to = "measure", values_to = "n") %>%
  st_as_sf() %>%
  ggplot() +
    geom_sf(aes(fill = n)) +
...
4.    We now have both occupancy as well as abundance data to start modelling. This example is for 1 species in 1 year. Real life data can have multiple years and species (data cube)

```{r}
summary_grid_props %>%
  mutate(expected_n = ifelse(expected_n < 0.4, NA, expected_n),
         abundance = as.character(ifelse(is.na(expected_n), 0, 
                                         ceiling(expected_n))),
         occupancy = ifelse(abundance == 0, "absent", "present")) %>%
  select(-expected_n) %>%
