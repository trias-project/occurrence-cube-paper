...
cum_mean_df2 %>%
  group_by(id) %>%
  mutate(sum = sum(mean_n$y)) %>%
  ungroup() %>%
  filter(sum != 0) %>%
  ggplot(aes(x = sim, y = mean_n$y, colour = as.factor(id), group = id)) +
    geom_errorbar(aes(ymin = mean_n$ymin, ymax = mean_n$ymax)) +
    geom_point(size = 0.05, colour = alpha("lightgrey", 0.1)) +
    scale_x_continuous(breaks = seq(0, 1000, 100)) +
    theme(legend.position = "") +
    labs(x = "number of simulations", 
         y = "median number of\noccurrences per grid")
...
cum_mean_df %>%
  ...
  ggplot(aes(x = sim, y = sum_abs_error)) +
      ...
      theme(legend.position = "") +
      labs(x = "number of simulations", 
           y = "total error on mean\nnumber of occurrences per grid")
```
...
occupancy_df <- do.call(rbind.data.frame, out)
...
occupancy_df %>%
  ggplot(aes(x = sim, y = p, colour = as.factor(id), group = id)) +
    ...
    theme(legend.position = "") +
    labs(x = "number of simulations", 
         y = "probability of occupancy per grid")
```
