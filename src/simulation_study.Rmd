...
```{r}
dat <- summary_grid_props %>%
  st_drop_geometry() %>%
  mutate(expected_n = ifelse(expected_n < 0.4, NA, expected_n),
         abundance = ifelse(is.na(expected_n), 0, ceiling(expected_n)),
         occupancy = ifelse(abundance == 0, 0, 1))
...
fit <- brm(bf(abundance ~ 1), data = dat, 
